...
1. Copy `starter/CONSTITUTION.md` → the workspace root `CLAUDE.md` — the folder standard that governs everything (code flat · thinking in the vault · gbrain prefix = recall lens). Every later file cascades inside it.
   - Also copy `starter/output-style.md` → workspace root `output-style.md` (the always-on voice/format), and add one line near the top of the root `CLAUDE.md`: *"Output style (always on): read `output-style.md` and hold it for every reply."* So every answer reads in the right voice by default.
